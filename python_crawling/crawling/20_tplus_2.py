...
driver.get("https://www.tplusmobile.com/view/phoneChrgeSystem/getEsimChrgeSystemList.do")

with open("../csv/" + str(carrierId) + "_tplus_2.csv", "w", newline="") as file:
    ...
    for plan in plans:
        name = plan.find_element(By.XPATH, 'ul/li/div[1]/strong').text.replace("\n", " ")
        call_minutes = getCallText(plan.find_element(By.XPATH, 'ul/li/div[2]/div[1]/div[2]/em').text)
        text_messages = getCallText(plan.find_element(By.XPATH, 'ul/li/div[2]/div[1]/div[3]/em').text)

        if call_minutes == '' or text_messages == '':
            continue
        data = plan.find_element(By.XPATH, 'ul/li/div[2]/div[1]/div[1]/em').text
        ...
        price_initial = getPrice(
            plan.find_element(By.XPATH, 'ul/li/div[2]/div[2]/div[4]/div[1]/em').text)

        mobile_carrier_id = carrierId
        # cta_url = plan.find_element(By.XPATH, 'div[2]/a').get_attribute('href')
        cta_url = "https://www.tplusmobile.com/view/phoneChrgeSystem/getEsimChrgeSystemList.do"
