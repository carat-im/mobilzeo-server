...
driver.get("https://www.skylife.co.kr/product/mobile/goods/PayPlan")

with open("../csv/" + str(carrierId) + "_ktskylife.csv", "w", newline="") as file:
    ...
    for i in range(1, 6):
        driver.find_element(By.XPATH, '//*[@id="cont-planFrugal"]/dl[' + str(i) + ']').click()
        time.sleep(1)

    driver.find_element(By.XPATH, '//*[@id="cont-planFrugal"]/dl[10]').click()
    time.sleep(1)

    plans = driver.find_elements(By.XPATH, '//*[@id="cont-planFrugal"]/dl/dd/div/div')
    print(len(plans))
    print(plans[0].text)

    for plan in plans:
        name = plan.find_element(By.XPATH, 'div[1]/p').text
        call_minutes = getCallText(plan.find_element(By.XPATH, 'div[2]/div/p[2]').text)
        text_messages = getCallText(plan.find_element(By.XPATH, 'div[2]/div/p[3]').text)
        data = plan.find_element(By.XPATH, 'div[2]/div/p[1]/span[2]').text

        data_per_month = getDataPerMonth(data)
        data_per_day = getDataPerDay(data)
        data_exhaustion_speed = getDataExhaustionSpeed(data)
        price_initial = getPrice(plan.find_element(By.XPATH, 'div[3]/p/b').text)

        mobile_carrier_id = carrierId
        cta_url = plan.find_element(By.XPATH, 'div[1]/a').get_attribute('href')
