...
driver.get("https://www.liivm.com/change/rateplan/rateplan/searchProduct")

with open("../csv/" + str(carrierId) + "_liivm.csv", "w", newline="") as file:
    ...
    time.sleep(1)
    plans = driver.find_elements("xpath", '//*[@id="all"]/ul/li')
    print(len(plans))
    print(plans[0].text)

    for plan in plans:
        name = plan.find_element(By.XPATH, 'div[1]/h5/strong').text
        call_minutes = getCallText(plan.find_element(By.XPATH, 'div[1]/dl[1]/dd[2]').text)
        text_messages = getCallText(plan.find_element(By.XPATH, 'div[1]/dl[1]/dd[3]').text)
        data = plan.find_element(By.XPATH, 'div[1]/dl[1]/dd[1]').text

        data_per_month = getDataPerMonth(data)
        data_per_day = getDataPerDay(data)
        data_exhaustion_speed = getDataExhaustionSpeed(data)

        price_initial = getPrice(plan.find_element(By.XPATH, 'div[1]/dl[2]/dd[2]').text)

        mobile_carrier_id = carrierId
        cta_url = plan.find_element(By.XPATH, 'div[2]/a[1]').get_attribute('href')
