...
driver.get("https://www.pindirectshop.com/pindirect-plan")

with open("../csv/" + str(carrierId) + "_pindirect.csv", "w", newline="") as file:
    ...
    for plan in plans:
        print(plan.text)
        name = plan.find_element(By.XPATH, 'h2').text.replace("\n", " ")

        callOrText = plan.find_elements(By.CLASS_NAME, 'css-19r3c6a')
        call_minutes = -1
        text_messages = -1
        if len(callOrText) > 1:
            call_minutes = getCallText(callOrText[0].text)
            text_messages = getCallText(callOrText[1].text)

        if call_minutes == '' or text_messages == '':
            continue
        data = plan.find_element(By.CLASS_NAME, 'css-xyewts').text
        ...
        price_initial = getPrice(
            plan.find_element(By.CLASS_NAME, 'css-1nbku3v').text)

        mobile_carrier_id = carrierId
        # cta_url = plan.find_element(By.XPATH, 'div[2]/a').get_attribute('href')
        cta_url = "https://www.pindirectshop.com/pindirect-plan"
