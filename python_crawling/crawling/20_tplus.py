...
def getDataPerMonth(dataText):
    dataText = dataText.replace("데이터\n", "").strip('월').replace("(GB)", "GB").replace("(MB)", "MB")
...
driver.get("https://www.tplusmobile.com/view/phoneChrgeSystem/getUsimChrgeSystemList.do")

with open("../csv/" + str(carrierId) + "_tplus.csv", "w", newline="") as file:
    ...
    for plan in plans:
        ...
        price_initial = getPrice(
            plan.find_element(By.XPATH, '*[@id="listUsim"]/li/div[2]/div[2]/div[4]/div[1]/em').text)

        mobile_carrier_id = carrierId
        # cta_url = plan.find_element(By.XPATH, 'div[2]/a').get_attribute('href')
        cta_url = "https://www.tplusmobile.com/view/phoneChrgeSystem/getUsimChrgeSystemList.do"
